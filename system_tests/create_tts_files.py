from pydub import AudioSegment
import numpy as np
import os
import requests

def create_wav(text, file_name):


    response            = requests.post(f"http://127.0.0.1:8002/synthesize/", json={"text": text})
    data                = response.json()
    samples = data['audio']
    samples = np.int16(samples / np.max(np.abs(samples)) * 32767)
    audio   = AudioSegment(samples.tobytes(),frame_rate=24000, channels=1, sample_width=2)
    audio   = audio.set_frame_rate(16000)
    audio.export(f"{os.getcwd()}/wav_commands/{file_name}", format="wav")

if __name__ == "__main__":

    # create_wav("Where am I located", "Where_am_I_located.wav")
    # create_wav("Open power screen", "Open_power_screen.wav")
    # create_wav("Go back to home screen", "Go_back_to_home_screen.wav")
    # create_wav("Missiles status", "Missiles_status.wav")
    # create_wav("Is the battery in charging mode ?", "Is_the_battery_in_charging_mode.wav")
    # create_wav("showing power screen", "show_power_screen.wav")
    # create_wav("showing inventory screen", "show_inventory.wav")
    # create_wav("showing navigation screen", "show_navigation.wav")
    # create_wav("showing overview screen", "show_overview.wav")
    create_wav("What is LMU power version ?", "What_is_LMU_power_version.wav")
